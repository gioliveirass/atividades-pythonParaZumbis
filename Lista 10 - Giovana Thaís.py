...
def end_other(a, b):
  a, b = a.lower(), b.lower()
  tamanhoA, tamanhoB = len(a), len(b)
  finalA, finalB = a[-tamanhoB:], b[-tamanhoA:]
  if a in finalB or b in finalA:
    return True
  else:
    return False
...
def soma_na_lista(n, lista):
  ...
  for i in range(len(lista)-1):
    if n == lista[i] + lista[i+1] and lista[i] != lista[i+1]:
      verificador += 1
  if verificador >= 1:
    return True
  else:
